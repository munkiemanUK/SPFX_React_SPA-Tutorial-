    ...
    NodeParser.prototype._parseBlockTag = function (tokenReader) {
        ...
        switch (tokenReader.peekPreviousTokenKind()) {
            ...
            case Token_1.TokenKind.Newline:
                break;
            default:
                return this._backtrackAndCreateError(tokenReader, marker, "tsdoc-at-sign-in-word" /* AtSignInWord */, 'A TSDoc tag must be preceded by whitespace');
        }
        ...
        if (tokenReader.peekTokenKind() !== Token_1.TokenKind.AsciiWord) {
            return this._backtrackAndCreateError(tokenReader, marker, "tsdoc-at-sign-without-tag-name" /* AtSignWithoutTagName */, 'Expecting a TSDoc tag name after the "@" character (or use a backslash to escape this character)');
        }
        ...
        if (tagName.length === 0) {
            return this._backtrackAndCreateError(tokenReader, marker, "tsdoc-malformed-inline-tag" /* MalformedInlineTag */, 'Expecting an inline TSDoc tag name immediately after "{@"');
        }
        ...
        switch (tokenReader.peekTokenKind()) {
            ...
            case Token_1.TokenKind.EndOfInput:
                break;
            default:
                return this._backtrackAndCreateError(tokenReader, marker, "tsdoc-text-after-tag" /* TextAfterTag */, 'A TSDoc tag must be followed by whitespace');
        }
    ...
    NodeParser.prototype._parseInlineTag = function (tokenReader) {
        ...
        if (spacingAfterTagNameExcerpt === undefined) {
            ...
            if (tokenReader.peekTokenKind() !== Token_1.TokenKind.RightCurlyBracket) {
                var failure = this._createFailureForToken(tokenReader, "tsdoc-text-after-tag" /* TextAfterTag */, 'Expecting a space after the TSDoc inline tag name');
                return this._backtrackAndCreateErrorRangeForFailure(tokenReader, marker, atSignMarker, '', failure);
            }
        }
        var done = false;
